...
- { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .vat-banner {
  background: #1a2a10; border: 1px solid #2a4a18; border-radius: 10px;
...
const [yieldRate, setYieldRate] = useState(80);   // 로스율 % (기본 80%)
...
const [vat,       setVat]       = useState(false); // 부가세 10%

// ── 수율 계산 ──
const yieldDecimal = yieldRate / 100;
const greenNeeded  = sellUnit / yieldDecimal;       // 필요 생두량 (g)
...
// ── 판매가 역산 ──
// 부가세 ON: 판매가 중 1/11이 세금 → 유효수익 = 판매가 × 10/11
// divisor = (10/11) - 수수료율 - 이익률   (VAT on)
// divisor = 1 - 수수료율 - 이익률          (VAT off)
...
// 로스율 힌트
const yieldHint = yieldRate >= 90 ? “라이트 (빠른 로스팅)”
: yieldRate >= 83 ? “미디엄”
: yieldRate >= 77 ? “미디엄 다크”
: “다크 (장시간 로스팅)”;
...
```
    ...
    <div className="card">
      ...
      {/* 로스율 슬라이더 */}
      <div className="field">
        <div className="field-lbl">로스율 (수율) <span>{yieldHint}</span></div>
        <div className="slider-wrap">
          <div className="slider-top">
            <span className="slider-val">{yieldRate}%</span>
            <span className="slider-hint">생두 1kg → 원두 {Math.round(yieldDecimal * 1000)}g</span>
          </div>
    ...
    <div className="card">
      ...
      <div className="toggle-row" onClick={() => setVat(v => !v)}>
        ...
        <span className="toggle-lbl">
          {vat ? "부가세 10% 포함 — 판매가에 반영됨" : "부가세 미적용 (간이과세자)"}
        </span>
      </div>
      {vat && (
    ...
    <div className="result-card">
      ...
      <div className="result-note">
        정확값 {divisor > 0 ? fmt(Math.round(rawPrice)) : "—"}원 → 100원 올림
        {vat ? " · 부가세 포함가" : ""}
      </div>
      ...
      <div className="result-grid">
        ...
        {vat && (
          <div className="r-item">
            <span className="r-lbl">부가세 납부액</span>
    ...
    <div className="formula">
      ...
      <div className="formula-body">
        ...
        생두원가 = {fmt(greenBean)} × {Math.round(greenNeeded)} ÷ 1000 = {fmt(greenCost)}원<br />
        로스팅비 = {fmt(roasting)} × {Math.round(greenNeeded)} ÷ 1000 = {fmt(roastCost)}원<br />
        커피원가 = <span className="hl">{fmt(coffeeCost)}원</span><br />
        총원가 = {fmt(totalCost)}원<br />
        {vat && <><span className="hl-g">부가세</span> = 판매가 ÷ 11 = {fmt(vatAmt)}원<br /></>}
        <br />
